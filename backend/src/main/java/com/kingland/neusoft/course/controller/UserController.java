...
public class UserController {
    ...
    public List<UserModel> searchUser(@RequestBody Map<String, Object> request){

        return this.userService.searchUser("%"+request.get("search")+"%");
    }
    /**
    ...
    public Map<String, Object> userUpdateData(@PathVariable("id") Long userId,@RequestBody Map<String, Object> userData){
        ...
        if(userService.updateUserModel(tools.toUserModel(userData)) ==0)
            return tools.responseJson(false, null, "更新用户数据失败");
        else 
            return tools.responseJson(true, null, "成功更新用户数据");
    }
